...
# load QCmetrics objects
for(i in c("rgSet", "QCmetrics", "detP", "bsCon", "mraw")){
  QCfile <- paste0(QCDir, "/", i, ".rdat")
  if(file.exists(QCfile)){
    load(file = QCfile)
  } else {
    stop(paste0(i, " rdat file does not exist."))
  }
}
```

# QC report for Illumina Infinium Mouse Methylation Array Data
...
```{r, definePlotVars,  warning=FALSE, message=FALSE, echo = FALSE}

# define variables to plot
plotVars <- c(projVar, "Chip_ID")

# extract chip ID from basename as Excel often converts to scientific notation
QCmetrics$Chip_ID <- as.character(substr(QCmetrics$Basename, 1, 12))

# ensure Individual_ID is factor
QCmetrics$Individual_ID <- factor(QCmetrics$Individual_ID)

# ensure age is numeric if provided
if("Age" %in% colnames(QCmetrics)){
  QCmetrics$Age <- as.numeric(QCmetrics$Age)
}

# trim white space from character variables
for(i in plotVars){
